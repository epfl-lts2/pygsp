...
def test_api_docstrings():
    ...
    for filename in files:
        try:
            ...
            if result.failed > 0:
                failures.append(f"{filename}: {result.failed}/{result.attempted} failed")
    ...
    significant_failures = failure_count > test_count * 0.1  # Allow up to 10% failures for formatting
    
    if significant_failures and failure_count > 0:
        failure_details = '\n'.join(failures[:10])  # Show first 10 failures
        pytest.fail(f"PyGSP docstring tests failed: {failure_count}/{test_count}\n{failure_details}")
...
def test_tutorial_docstrings():
    ...
    if failure_count > 0 and failure_count > test_count * 0.2:  # Allow up to 20% failures for tutorials
        failure_details = '\n'.join(failures[:5])  # Show first 5 failures
        pytest.fail(f"PyGSP tutorial docstring tests failed: {failure_count}/{test_count}\n{failure_details}")


@pytest.mark.skip(reason="External library doctests are version-dependent and not our responsibility")
...
def test_basic_docstring_examples():
    ...
    if examples:
        ...
        test = doctest.DocTest(examples, globs, "basic_test", "basic_test", 0, example_code)
        result = runner.run(test)
        
        if result.failed > 0:
            pytest.fail(f"Basic docstring example failed: {result.failed}/{result.attempted}")