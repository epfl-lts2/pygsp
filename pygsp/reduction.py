...
from pygsp import filters, graphs, utils
...
def interpolate(G, f_subsampled, keep_inds, order=100, reg_eps=0.005, **kwargs):
    ...
    green_kernel = getattr(
        G.mr, "green_kernel", filters.Filter(G, lambda x: 1.0 / (reg_eps + x))
    )
...
def kron_reduction(G, ind):
    ...
    if isinstance(G, graphs.Graph):
        if G.lap_type != "combinatorial":
            msg = f"Unknown reduction for {G.lap_type} Laplacian."
    ...
    if isinstance(G, graphs.Graph):
        ...
        Gnew = graphs.Graph(
            Wnew, coords=coords, lap_type=G.lap_type, plotting=G.plotting
        )
    else:
        Gnew = Lnew
...
def pyramid_analysis(Gs, f, **kwargs):
    ...
    if len(h_filters) == 1:
        ...
        message = f"The number of filters must be one or equal to {levels}."
...
def _pyramid_single_interpolation(G, ca, pe, keep_inds, h_filter, **kwargs):
    ...
    if use_landweber:
        ...
        for iteration in range(landweber_its):
            ...
            x_up = sparse.csr_matrix(
                (z_delt, (range(nb_ind), [1] * nb_ind)), shape=(N, 1)
            )
            ...
            next_up = sparse.csr_matrix(
                (next_term, (keep_inds, [1] * nb_ind)), shape=(N, 1)
            )
            x += (
                landweber_tau * _analysis(filt, x_up - next_up, **kwargs)
                + z_delt[nb_ind:]
            )
        ...
        finer_approx = linalg.spsolve(
            Ta.T * Ta, Ta.T * np.concatenate((ca, pe), axis=0)
        )
...
def tree_multiresolution(
    ...
    for lev in range(Nlevel):
        ...
        if reduction_method == "unweighted":
            ...
            # old_W(old_parents_of_non_root_keep_inds,old_grandparents_of_non_root_keep_inds);
            new_weights = 1.0 / (
                1.0 / old_weights_to_parents + 1.0 / old_weights_parents_to_grandparents
            )
            new_weights = np.concatenate([new_weights, new_weights])
        ...
        # Store new tree
        Gtemp = graphs.Graph(
            new_W, coords=Gs[lev].coords[keep_inds], limits=G.limits, root=new_root
        )
