...
class SphereDodecahedron(NNGraph):
    r"""Spherical-shaped graph based on the projection of the dodecahedron (NN-graph).
    Code inspired by Max Jiang [https://github.com/maxjiang93/ugscnn/blob/master/meshcnn/mesh.py]
    ...
    See Also
    --------
    SphereIcosahedron, SphereHealpix, SphereEquiangular

    Notes
    ------
    The dodecahedron is the dual of the icosahedron. Thus the pixels in this graph represent either the vertices \
    of the dodecahedron, or the faces of the icosahedron.

    Examples
    --------
    >>> import matplotlib.pyplot as plt
    >>> from mpl_toolkits.mplot3d import Axes3D
    >>> G = graphs.SphereDodecahedron(level=1)
    >>> fig = plt.figure()
    >>> ax1 = fig.add_subplot(121)
    >>> ax2 = fig.add_subplot(122, projection='3d')
    >>> _ = ax1.spy(G.W, markersize=1.5)
