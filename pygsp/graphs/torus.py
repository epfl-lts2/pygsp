...
class Torus(Graph):
    ...
    def __init__(self, Nv=16, Mv=None, **kwargs):
        ...
        for i in range(Mv):
            i_inds[i * K + tmpK] = i * Nv + np.concatenate(
                (np.array([Nv - 1]), tmpNv1, tmpNv)
            )

            j_inds[i * K + tmpK] = i * Nv + np.concatenate(
                (tmpNv, np.array([Nv - 1]), tmpNv1)
            )

        tmp2Nv = np.arange(2 * Nv, dtype=int)

        for i in range(Mv - 1):
            i_inds[K * Mv + i * 2 * Nv + tmp2Nv] = np.concatenate(
                (i * Nv + tmpNv, (i + 1) * Nv + tmpNv)
            )

            j_inds[K * Mv + i * 2 * Nv + tmp2Nv] = np.concatenate(
                ((i + 1) * Nv + tmpNv, i * Nv + tmpNv)
            )

        i_inds[K * Mv + (Mv - 1) * 2 * Nv + tmp2Nv] = np.concatenate(
            (tmpNv, (Mv - 1) * Nv + tmpNv)
        )

        j_inds[K * Mv + (Mv - 1) * 2 * Nv + tmp2Nv] = np.concatenate(
            ((Mv - 1) * Nv + tmpNv, tmpNv)
        )

        W = sparse.csc_matrix(
            (np.ones(K * Mv + J * Nv), (i_inds, j_inds)), shape=(Mv * Nv, Mv * Nv)
        )
        ...
        coords = np.concatenate(
            (
                np.reshape(xtmp, (Mv * Nv, 1), order="F"),
