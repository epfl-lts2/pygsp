...
class Community(Graph):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if min_comm * Nc > N:
            raise ValueError(
                "The constraint on minimum size for communities is unsolvable."
            )
        ...
        if comm_sizes is None:
            mandatory_labels = np.tile(
                np.arange(Nc), (min_comm,)
            )  # min_comm labels for each of the Nc communities
            remaining_labels = rng.choice(
                Nc, N - min_comm * Nc
            )  # random choice for the remaining labels
            info["node_com"] = np.sort(
                np.concatenate((mandatory_labels, remaining_labels))
            )
        else:
        ...
        if comm_density is not None:
            ...
            self.logger.info(f"Constructed using community density = {comm_density}")
        ...
        # Coordinates #
        info["com_coords"] = info["world_rad"] * np.array(
            list(
                zip(
        ...
        for i in range(Nc):
            ...
            if comm_density is not None:
                ...
                map(
                    lambda row: map(
                        lambda elm: pairs_set.add((min(row[0], elm), max(row[0], elm))),
                        row[1:],
                    ),
                    indices,
                )
        ...
        super().__init__(W, coords=coords, **kwargs)
    ...
    def _get_extra_repr(self):
        attrs = {
            ...
            "size_ratio": f"{self.size_ratio:.2f}",
            "world_density": f"{self.world_density:.2f}",
        }
        if self.comm_density is not None:
            attrs["comm_density"] = f"{self.comm_density:.2f}"
        elif self.k_neigh is not None:
            attrs["k_neigh"] = self.k_neigh
        else:
            attrs["epsilon"] = f"{self.epsilon:.2f}"
